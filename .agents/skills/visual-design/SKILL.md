...
- **Material Visual Decision**: A choice that establishes or substantially changes the brand palette, typography system, logo treatment, illustration or photographic direction, major layout paradigm, theme strategy, visual density, public-facing aesthetic, or cross-product design language. Accessibility and platform requirements normally act as constraints. Escalation is required only when satisfying them would require changing approved behaviour, scope, brand direction, or another unresolved material decision.
  - **Under a Delegated Creative Authority envelope**: when the current task frame records a delegation covering this skill (see `task-framing`'s [Delegated Creative Authority](../task-framing/SKILL.md) section), the delegated categories — typically the type system and scale, palette and theme strategy, major layout paradigm, visual density, imagery direction, and this project's public-facing aesthetic — are made as authorized bounded decisions recording the delegation as their authority basis, rather than escalated. Categories the envelope does not name still escalate. A delegation never relaxes accessibility or platform constraints, never authorizes changing approved product behaviour or hierarchy, and never authorizes cross-product design-language changes beyond the delegated project surface. Where an approved `creative-direction` thesis exists, the delegation operates within it — it does not authorize departing from the approved thesis.
...
- [ ] **No Silently Invented Brand Decisions**: Unresolved brand or visual-system decisions are not silently invented. Where a Delegated Creative Authority envelope authorizes a visual-system decision, the delegation is recorded as its authority basis and the decision stays within the categories the envelope names.
